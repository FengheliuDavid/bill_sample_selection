...
GPT-4o to classify it against the Wage Hour & Benefits category using the
prompt in 8_llm_categorize_VT27_bills_prompt_06_18.md. Full bill text is read
from INPUT/SAMPLE_1000/llm_input_texts_no_prefilter/. Outputs to
OUTPUT/llm_bill_VT27_gpt4o_06_18.csv.
...
OUTPUT_CSV = SCRIPT_DIR / "OUTPUT" / "llm_bill_VT27_gpt4o_06_18.csv"
...
# Prefix GPT output columns with gpt_ to avoid collision with existing
# bill_type / reason columns from the upstream Gemma classification.
out_fields = list(rows[0].keys()) + ["gpt_categories", "gpt_bill_type", "gpt_reason"]
...
for i, row in enumerate(rows):
    ...
    for attempt in range(1, 4):
        try:
            ...
            results.append({**row, "gpt_categories": cats_str, "gpt_bill_type": bill_type, "gpt_reason": reason})
        ...
        results.append({**row, "gpt_categories": None, "gpt_bill_type": None, "gpt_reason": "API error"})
        new_since_checkpoint += 1
...
# Quick summary
matched     = sum(1 for r in results if r.get("gpt_categories"))
no_match    = sum(1 for r in results if not r.get("gpt_categories"))
enforcement = sum(1 for r in results if r.get("gpt_bill_type") == "enforcement")
tc_intro    = sum(1 for r in results if r.get("gpt_bill_type") == "tax_credit_introduction")
tc_remove   = sum(1 for r in results if r.get("gpt_bill_type") == "tax_credit_removal")
