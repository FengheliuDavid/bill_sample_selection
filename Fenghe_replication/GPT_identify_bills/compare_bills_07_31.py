#!/usr/bin/env python3
"""
Runs bills flagged by Qwen (07_31 run) through both GPT-4o and Claude Sonnet.
Both models use the same prompt: 8_llm_categorize_VT27_bills_prompt_07_31.md.
Change from 07_15: Davis-Bacon bullet removed from the DO qualify section.
...
Output:
  - OUTPUT/llm_bill_VT27_compare_07_31.csv
  - OUTPUT/compare_07_31_checkpoint.csv  (auto-deleted on completion)
  - ../manual_review_07_31.xlsx          (GPT-4o vs Sonnet disagreements only)
"""
...
OUTPUT_CSV = SCRIPT_DIR / "OUTPUT" / "llm_bill_VT27_compare_07_31.csv"
CHECKPOINT = SCRIPT_DIR / "OUTPUT" / "compare_07_31_checkpoint.csv"
REVIEW_XLS = REPO_DIR / "manual_review_07_31.xlsx"
...
# ---------------------------------------------------------------------------
# Generate manual review xlsx (GPT-4o vs Sonnet disagreements)
# ---------------------------------------------------------------------------
disagree_rows = [
    r for r in results
    if is_flagged(r["category_4o"]) != is_flagged(r["category_sonnet"])
]

print(f"\nGPT-4o vs Sonnet disagree: {len(disagree_rows)} bills -> {REVIEW_XLS}")
...
for row_idx, r in enumerate(disagree_rows, 2):
    ...
    for col_idx, (_, field, _, group) in enumerate(columns, 1):
        ...
        # Shade the winning model column lightly
