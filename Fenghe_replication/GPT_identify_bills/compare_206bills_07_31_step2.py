#!/usr/bin/env python3
"""
Step-2 pass: runs the 206 bills flagged by Qwen step-1 (07_31) through
GPT-4o, Claude Sonnet, and Claude Opus using the detailed-rules prompt.
...
Output:
  - OUTPUT/llm_bill_VT27_compare_07_31_step2.csv
  - OUTPUT/compare_07_31_step2_checkpoint.csv  (auto-deleted on completion)
  - ../manual_review_07_31_step2.xlsx          (any model disagreement)
  - OUTPUT/api_cost_07_31_step2.txt            (TRUE cost from API usage)
"""
...
OUTPUT_CSV = SCRIPT_DIR / "OUTPUT" / "llm_bill_VT27_compare_07_31_step2.csv"
CHECKPOINT = SCRIPT_DIR / "OUTPUT" / "compare_07_31_step2_checkpoint.csv"
REVIEW_XLS = REPO_DIR / "manual_review_07_31_step2.xlsx"
COST_TXT   = SCRIPT_DIR / "OUTPUT" / "api_cost_07_31_step2.txt"
...
with open(STEP1_CSV, encoding="utf-8") as f:
    for r in csv.DictReader(f):
        if str(r.get("category_ids", "")).strip() in ("1", "1.0"):
...
# Helpers
# ---------------------------------------------------------------------------
def build_user_message(bill_id: str, m: dict, step1_reason: str) -> str:
    ...
    return (
        ...
        f"The first-pass model flagged this bill with the following reason:\n"
        f"> {step1_reason}"
    )
...
for idx, (bill_id, s1_row) in enumerate(bill_list, 1):
    ...
    user_msg = build_user_message(bill_id, m, step1_reason)
...
cost_lines = f"""API Cost — compare_206bills_07_31_step2.py
==========================================
Run completed: {now} (local time)
Script:        GPT_identify_bills/compare_206bills_07_31_step2.py
